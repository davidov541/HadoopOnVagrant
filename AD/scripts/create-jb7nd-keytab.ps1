...
Try {
  NEW-ADOrganizationalUnit -name "ServiceAccounts" -path "OU=IT-Services,DC=AD,DC=DAVIDMCGINNIS,DC=NET"
} Catch {}
...
New-ADUser -SamAccountName $identity -GivenName "JBoss7 SSO" -Surname "JBoss7 SSO" -Name $identity `
  ...
  -Path "OU=ServiceAccounts,OU=IT-Services,DC=AD,DC=DAVIDMCGINNIS,DC=NET" `
...
$servicePrincipalName = 'HTTP/' + $hostname + '.ad.davidmcginnis.net@AD.DAVIDMCGINNIS.NET'
& ktpass -out c:\vagrant\resources\$identity.keytab -princ $servicePrincipalName -mapUser "AD\$identity" -mapOp set -pass $password  -crypto RC4-HMAC-NT
